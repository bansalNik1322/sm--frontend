...
import { useApp, useLoading, useRequest, useSettings } from '@/components/App';
...
function Index(props: PROPS) {
  ...
  const [fontFamilyList, _] = useState([
    'Inter',
    'Roboto',
    'Poppins',
    'auto',
  ...
  // const handleFormikChange = (fieldName: string, fieldValue: any) => {
  ...
  const handleFileChange = (event: React.ChangeEvent, name: string) => {
    ...
    if (selectedFile) {
      dispatch({ [name]: URL.createObjectURL(selectedFile) });
      dispatch({ [name + '_files']: selectedFile });
    }
  };
  ...
  function handleImageDrop(event: React.DragEvent<HTMLLabelElement>, name: string) {
    ...
    if (droppedFile) {
      dispatch({ [name]: URL.createObjectURL(droppedFile) });
      dispatch({ [name + '_files']: droppedFile });
    }
  }
  ...
  const bannerURL = useMemo(() => {
    if (state?.bannerImg) {
      ...
      return '/assets/images/upload-cloud-02.svg';
    }
            ...
            onSubmit={async values => {
              ...
              if (req) {
                ...
                updateGlobalSetting({
                  ...
                  logo: `/spimages/${req?.columnData?.logo}`,
                  banner: `/spimages/${req?.columnData?.banner}`,
            ...
            {({ handleSubmit, handleChange, values, errors, setFieldValue }) => (
              <Form noValidate onSubmit={handleSubmit}>
                      ...
                      <div className="file-upload">
                        ...
                        >
                          ...
                          <div className="upload-icon">
                            <Image alt="uploadIcon" height={20} width={20} src="/assets/images/upload-cloud-02.svg" />
                          </div>
                        ...
                        <div className="logo-icon">
                          <Image loader={logoImageLoader} alt="logoIcon" height={29} width={29} src={logoURL} />
                        </div>
                      </div>
                    </div>
                  </div>
                ...
                <div className="settingFooter">
                  <Button type="reset" className="OutlineBtn mr10 ">
                    Cancel
                  </Button>
